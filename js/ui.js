...
const UI = (() => {
  ...
  function renderRoundEnd(state, myId) {
    ...
    header.innerHTML = `<span>PLAYER</span><span>POS</span><span>COST</span><span>R/C</span><span>PAYOFF</span>`;
    wrap.appendChild(header);

    // Sort by payoff descending
    const rows = Object.entries(standings).sort((a, b) => b[1].payoff - a[1].payoff);
    rows.forEach(([id, s]) => {
      const isMe = id === myId;
      const div = document.createElement('div');
