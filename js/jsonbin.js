...
const JSONBIN = (() => {
  ...
  /**
   * Delete a game bin entirely from JSONBin.
   * Called by host after game finishes.
   */
  async function deleteGameBin(binId) {
    await _request('DELETE', `/b/${binId}`);
  }

  /**
   * Remove a game code entry from the index bin.
   */
  async function unregisterGame(gameCode) {
    const index = await readIndex();
    delete index[gameCode.toUpperCase()];
    await _request('PUT', `/b/${_indexBinId()}`, index);
  }
